...
public class MovieChartDAO {	
	 ...
	 * @throws SQLException
	 */
	public List<MovieVO> selectShowingMovieList(String isChecked) throws SQLException {
		List<MovieVO> mVOList = new ArrayList<MovieVO>();
		MovieVO mVO = null;
		DbConnection dbcon = DbConnection.getInstance();
		Connection con = null;
		...
		try {
			...
			// 3. SQL 쿼리 준비
			StringBuilder sbQueryString = new StringBuilder();
			sbQueryString.append("SELECT M.MOVIE_CODE, M.MOVIE_TITLE, M.MOVIE_RELEASE_DATE, M.MOVIE_POSTER_PATH, M.MOVIE_RATING, M.MOVIE_SCREENING_STATUS, ");
			sbQueryString.append("ROUND(NVL(SUM(R.review_score) / NULLIF(COUNT(R.REVIEW_NUMBER), 0), 0), 2) AS AVG_REVIEW ");
			sbQueryString.append("FROM MOVIE M ");
			sbQueryString.append("LEFT JOIN REVIEW R ON M.MOVIE_CODE = R.MOVIE_CODE ");
			sbQueryString.append("WHERE M.MOVIE_SCREENING_STATUS = ? ");
			sbQueryString.append("GROUP BY M.MOVIE_CODE, M.MOVIE_TITLE, M.MOVIE_RELEASE_DATE, M.MOVIE_POSTER_PATH, M.MOVIE_RATING,M.MOVIE_SCREENING_STATUS ");
			sbQueryString.append("ORDER BY AVG_REVIEW DESC, M.MOVIE_RELEASE_DATE DESC");
			
			
			String selectQuery = sbQueryString.toString();
			pstmt = con.prepareStatement(selectQuery);
			pstmt.setString(1, isChecked);
			// 4. 쿼리 실행 및 결과 처리
			rs = pstmt.executeQuery();
